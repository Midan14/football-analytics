...
async def get_teams(
    ...
    try:
        ...
        # cache_key = f"teams:list:{hash(str({
        #     'league_id': league_id, 'country': country, 'search': search,
        #     'skip': skip, 'limit': limit, 'sort_by': sort_by, 'sort_order': sort_order,
        #     'include_stats': include_stats, 'include_form': include_form
        # }))}"

        # Verificar cache
        # cached_result = await redis_client.get(cache_key) # Comentado para evitar error si redis_client no está listo
        # if cached_result and not include_form:  # No cache si incluye forma (datos dinámicos)
        ...
        # if not include_form: # Comentado para evitar error si redis_client no está listo
...
async def get_team(
    ...
    try:
        ...
        # Verificar cache
        # cached_result = await redis_client.get(cache_key) # Comentado
        # if cached_result and not include_recent_matches:  # No cache para partidos recientes
        #     return json.loads(cached_result)
        ...
        # if not include_recent_matches: # Comentado
