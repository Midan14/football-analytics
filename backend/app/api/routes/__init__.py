...
# Router de MCP (Model Context Protocol) - NUEVO
try:
    from . import mcp

    # Verificar que el módulo tiene el atributo router
    if getattr(mcp, "router", None) is not None:
        routers_config.append(
            {
                "router": mcp.router,
                "prefix": "/mcp",
                ...
                "description": "Integración con servidores "
                "MCP para contexto "
                "adicional",
            }
        )
        successfully_imported.append("mcp")
        logger.debug("✅ Router de MCP cargado")
    else:
        failed_imports.append(("mcp", ROUTER_NOT_FOUND_MSG))
        logger.warning("⚠️  Router de MCP sin atributo router")
except ImportError as e:
    failed_imports.append(("mcp", str(e)))
    logger.warning("⚠️  Router de MCP no encontrado")
except AttributeError as e:
    failed_imports.append(("mcp", f"AttributeError: {str(e)}"))
    logger.warning("⚠️  Error de atributo en router de MCP")
